...
exports.pdfDocument = async (req, res) => {
  const quiz = await Quiz.findById(req.params.id);
  // Trang quản trị cần xem PDF ngay sau khi upload, trước khi bấm "Lưu đề PDF".
  // Học viên chỉ được xem sau khi đề đã bật pdf_exam_mode.
  const isAdminPreview = req.originalUrl.startsWith('/admin/');
  if (!quiz || (!quiz.pdf_exam_mode && !isAdminPreview)) return res.status(404).end();
