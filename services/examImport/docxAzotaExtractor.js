...
async function normalizeFormulaPng(dataUrl) {
  ...
  try {
    ...
    const pad = Math.max(2, Math.round((maxY - minY + 1) * 0.08));
    ...
    const out = createCanvas(Math.max(1, Math.round(cropW * scale)), Math.max(1, Math.round(cropH * scale)));
    const outCtx = out.getContext('2d');
    outCtx.drawImage(src, minX, minY, cropW, cropH, 0, 0, out.width, out.height);
    return out.toDataURL('image/png');
  } catch (error) {
    console.warn('[exam-import] formula crop failed:', error.message);
    return dataUrl;
  }
}
